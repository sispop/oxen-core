...
*Setup instructions for a Gitian build of Sispop.*

Gitian is the deterministic build process that is used to build the Sispop CLI
...
```bash
cp sispop/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```bash
./gitian-build.py --setup sispop-user x.x.x
```

Where `sispop-user` is your Github name and `x.x.x` is the version tag you want to build.
If you are using docker, run it with:

```bash
./gitian-build.py --setup --docker sispop-user v3.0.4
```
...
```bash
git clone git@github.com:sispop-project/gitian.sigs.git
git remote add sispop-user git@github.com:sispop-user/gitian.sigs.git
```
...
```bash
./gitian-build.py --detach-sign --no-commit -b sispop-user v3.0.4
```
...
```bash
export NAME=sispop-user
export VERSION=v3.0.4
gpg --output $VERSION-linux/$NAME/sispop-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/sispop-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/sispop-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/sispop-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/sispop-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/sispop-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[sispop-project/gitian.sigs](https://github.com/sispop-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/sispop-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/sispop-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sispop-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/sispop-project/sispop -b <branch> <commit hash>
```

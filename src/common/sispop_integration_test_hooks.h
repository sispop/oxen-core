...
#ifndef SISPOP_INTEGRATION_TEST_HOOKS_H
#define SISPOP_INTEGRATION_TEST_HOOKS_H
...
#endif // SISPOP_INTEGRATION_TEST_HOOKS_H

//
// CPP Implementation
//
#ifdef SISPOP_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
#include <string.h>
#include <assert.h>
#include <chrono>
#include <thread>
...
, "sispop-default-integration-test-mem-name"
, false
};
...
void sispop::use_standard_cout()   { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_std_cout); }
void sispop::use_redirected_cout() { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_redirected_cout.rdbuf()); }

void sispop::init_integration_test_context(const std::string &base_name)
{
  ...
  while (global_stdin_shared_mem->Open() != 0)
  {
    ...
    {
      once_only = false;
      printf("Sispop Integration Test: Shared memory %s has not been created yet, blocking ...\n", global_stdin_shared_mem->Path().c_str());
    }
  }
  ...
  if (!global_stdin_semaphore_handle)  fprintf(stderr, "Sispop Integration Test: Failed to initialise global_stdin_semaphore_handle\n");
  if (!global_stdout_semaphore_handle) fprintf(stderr, "Sispop Integration Test: Failed to initialise global_stdout_semaphore_handle\n");
  if (!global_stdout_ready_semaphore) fprintf(stderr, "Sispop Integration Test: Failed to initialise global_stdout_ready_semaphore_handle\n");
  if (!global_stdin_ready_semaphore) fprintf(stderr, "Sispop Integration Test: Failed to initialise global_stdin_ready_semaphore_handle\n");

  printf("Sispop Integration Test: Hooks initialised into shared memory, %s, %s, %s, %s, %s, %s\n",
      stdin_name.c_str(),
      stdout_name.c_str(),
...
void sispop::deinit_integration_test_context()
{
...
void write_to_stdout_shared_mem(char const *buf, int buf_len)
{
  ...
  assert(buf_len < sispop::fixed_buffer::SIZE);
...
std::vector<std::string> sispop::separate_stdin_to_space_delim_args(sispop::fixed_buffer const *cmd)
{
...
sispop::fixed_buffer sispop::read_from_stdin_shared_mem()
{
...
void sispop::write_redirected_stdout_to_shared_mem()
{
  boost::unique_lock<boost::mutex> scoped_lock(integration_test_mutex);
...
#endif // SISPOP_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
